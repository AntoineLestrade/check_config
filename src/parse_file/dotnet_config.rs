...
pub fn parse(string_content: &String,
             parsing_options: &super::super::parser_options::ParsingOptions)
    ...
    let re_server_value: regex::Regex =
        regex::Regex::new(parsing_options.regex_server_value.as_str()).unwrap();
    let re_db_value: regex::Regex =
        regex::Regex::new(parsing_options.regex_database_value.as_str()).unwrap();
                ...
                Ok(XmlEvent::StartElement { name, attributes, .. }) => {
                    if name.local_name == "add" {
                        ...
                        for attr in attributes.into_iter() {
                            ...
                            match val {
                                "name" => {
                                    pfr.cs_name = attr.value;
                                }
                                "connectionString" => {
                                    buff_value = attr.value;
                                }
                                _ => {}
                            };
                        }
                        if let Some(val) = RE_CS_VALUE.captures(&*buff_value) {
                            ...
                            if re_server_value.is_match(val.get(1).unwrap().as_str()) !=
                               parsing_options.regex_server_inverse {
                                pfr.is_good = false;
                            }
                            if re_db_value.is_match(val.get(2).unwrap().as_str()) !=
                               parsing_options.regex_database_inverse {
                                pfr.is_good = false;
                            }
                        }
                        result.push(pfr);
...
mod tests {
    ...
    <configuration xmlns="http://schemas.microsoft.com/.NetConfiguration/v2.0">
        ...
        let config = super::super::super::parser_options::ParsingOptions {
            regex_server_value: r"(?i)(\.|dabel69(\.corp\.altengroup\.dir)?)\\sqlexpress"
                .to_string(),
        ...
        match result {
            ...
            Err(_) => {
                assert!(false,
                        "Config file is considered as wrong, it should be good");
            }
        }
    }
}
